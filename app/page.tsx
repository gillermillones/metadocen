      ...
      <div className="mt-4 flex grow flex-col gap-4 md:flex-row">
        <div className="flex flex-col justify-start gap-6 rounded-lg bg-gray-50 px-6 py-10 md:w-2/5 md:px-20">
            <p className={`${lusitana.className} antialiased text-xl text-gray-800 md:text-3xl md:leading-normal`}>
              <strong>Bienvenido a Metadocen.</strong> Aqui podras obtener 
              metadatos a medida para tus recursos docentes segun la Norma 71362:2020, 
              o navegar a traves de muchos creados por otros profesionales. Empieza aqui:
            </p>
          <Link
            href="/login"
          ...
          >
            <span>Inicia sesion</span> <ArrowRightIcon className="w-5 md:w-6 ml-2 transition delay-150 duration-300 ease-in-out group-hover/login:translate-x-6" />
          </Link>
          <Link
            href="/register"
          ...
          >
            <span>Registrate</span> <ArrowRightIcon className="w-5 ml-2 md:w-6 transition delay-150 duration-300 ease-in-out group-hover/signup:translate-x-6" />
          </Link>
        </div>
